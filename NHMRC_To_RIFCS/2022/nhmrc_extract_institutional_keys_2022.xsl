 ...
 <!-- this stylesheet attempt to match all values in the Admin_Institution column from the supplied NHMRC grant data 
  ...
  -->
    ...
    <xsl:variable name="queryroot" 
        select="'http://researchdata.ands.org.au/registry/services/api/getRIFCS?q=class:(party) AND type:(group) AND title_search:'"/>
    ...
    <xsl:template match="/root">
        ...
        <xsl:element name="institutions">                           
            <xsl:for-each-group select="row" group-by="Admin_Institution">
                ...
                <xsl:variable name="query" select="concat($queryroot,'(',$searchterms,')')"/>
                
                <xsl:variable name="institution" select="document($query)"/>
                <xsl:text>&#xA;</xsl:text>
