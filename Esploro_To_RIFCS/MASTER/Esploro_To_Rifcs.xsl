...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    xmlns:oai="http://www.openarchives.org/OAI/2.0/" xmlns:custom="http://custom.nowhere.yet"
    ...
    <xsl:param name="global_oaiIdPrefix" select="'{override required}'"/>
    ...
    <xsl:template match="/">
        <registryObjects xmlns="http://ands.org.au/standards/rif-cs/registryObjects"
            ...
            <xsl:apply-templates select="//oai:record/oai:metadata/*:record/*:data"
                mode="collection"/>
            <xsl:apply-templates select="//oai:record/oai:metadata/*:record/*:data" mode="party"/>
            <xsl:apply-templates
                select="//oai:record/oai:metadata/*:record/*:data/*:fundingreferenceList/*:fundingreference"
                mode="activity"/>
        </registryObjects>
    </xsl:template>
    ...
    <xsl:template match="data" mode="collection">
        ...
        <registryObject>
            ...
            <xsl:element name="collection">
                ...
                <xsl:apply-templates select="@todo[string-length(.) > 0]"
                ...
                <xsl:apply-templates
                    select="*[starts-with(local-name(), 'identifier')][string-length() > 0]"
                    mode="collection_identifier"/>

                <xsl:apply-templates select="title[string-length(.) > 0]" mode="collection_name"/>

                <xsl:choose>
                    <xsl:when test="count(identifier.doi[string-length(.) > 0][1]) > 0">
                        <xsl:apply-templates select="identifier.doi[string-length(.) > 0][1]"
                            mode="collection_location_doi"/>
                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:apply-templates
                            select="originalRepository/coverpage-url[string-length(.) > 0]"
                ...
                <xsl:if test="string-length(coverpage-url) = 0">
                    <xsl:apply-templates select="submission-path[(string-length(.) > 0)]"
                        mode="collection_location"/>
                </xsl:if>

                <xsl:apply-templates
                    select="creators/creator[(string-length(givenname) > 0) or (string-length(familyname) > 0)]"
                    mode="collection_relatedObject_individual"/>

                <xsl:apply-templates select="creators/creator[(string-length(organization) > 0)]"
                    mode="collection_relatedObject_organization"/>

                <xsl:apply-templates
                    select="contributors/contributor[(string-length(givenname) > 0) or (string-length(familyname) > 0)]"
                    mode="collection_relatedObject_individual"/>

                <xsl:apply-templates
                    select="contributors/contributor[(string-length(organization) > 0)]"
                    mode="collection_relatedObject_organization"/>

                <xsl:apply-templates select="keywords[string-length(.) > 0]"
                    mode="collection_subject"/>

                <xsl:apply-templates select="subject.anzfor[string-length(.) > 0]"
                    mode="collection_subject_anzsrc_for"/>

                <xsl:apply-templates select="description.abstract[string-length(.) > 0]"
                    mode="collection_description_full"/>

                <xsl:apply-templates select="description.other[string-length(.) > 0]"
                    mode="collection_description_full"/>

                <xsl:apply-templates
                    select="geoLocation/geoLocationPoints/geoLocationPoint[string-length(.) > 0]"
                    mode="collection_coverage_spatial_point"/>

                <xsl:apply-templates
                    select="geoLocation/geoLocationAddress/geoLocationAddres[string-length(.) > 0]"
                ...
                <xsl:apply-templates
                    select="fundingreferenceList/fundingreference[string-length(.) > 0]"
                    mode="collection_relatedInfo"/>

                <xsl:apply-templates select="relationships/relationship[string-length(.) > 0]"
                    mode="collection_relatedInfo"/>

                <xsl:apply-templates select="copyright[string-length(.) > 0]"
                    mode="collection_rights_statement"/>

                <xsl:apply-templates select="license[string-length(.) > 0]"
                    mode="collection_rights_license"/>

                <xsl:apply-templates select="rightsList/rights/rights[string-length(.) > 0]"
                    mode="collection_rights_access"/>

                <xsl:apply-templates select="date.submitted[string-length(.) > 0]"
                    mode="collection_dates_submitted"/>

                <xsl:apply-templates select="date.available[string-length(.) > 0]"
                    mode="collection_dates_available"/>

                <xsl:apply-templates select="date.published[string-length(.) > 0]"
                    mode="collection_dates_issued"/>

                <xsl:apply-templates select="date.collected[string-length(.) > 0]"
                    mode="collection_dates_collected"/>
    ...
    <xsl:template match="oai:identifier" mode="collection_identifier_oai">
        <identifier type="{$global_acronym}">
            <xsl:choose>
                <xsl:when test="contains(., $global_oaiIdPrefix)">
                    <xsl:value-of select="substring-after(., $global_oaiIdPrefix)"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:value-of select="normalize-space(.)"/>
                </xsl:otherwise>
    ...
    <xsl:template match="*" mode="collection_relatedObject_individual">
        <xsl:if test="(string-length(givenname) > 0) or (string-length(familyname) > 0)">
            <relatedObject>
                <key>
                    <xsl:value-of
                        select="custom:registryObjectKeyFromString(concat(givenname, familyname))"/>
                </key>
        ...
        <xsl:if test="(string-length(longitude) > 0) and (string-length(latitude) > 0)">
            <coverage>
                <spatial type="kmlPolyCoords">
                    <xsl:value-of
                        select="concat(normalize-space(longitude), ',', normalize-space(latitude))"
                    />
                </spatial>
                <spatial type="text">
                    <xsl:value-of select="normalize-space(displayString)"/>
        ...
        <relatedInfo type="activity">
            <identifier>
                <xsl:attribute name="type">
                    <xsl:choose>
                        <xsl:when
                            test="contains(fundername, 'Australian Research Council') or (contains(fundername, 'ARC'))">
    ...
    <xsl:template match="relationship" mode="collection_relatedInfo">
        <xsl:if test="
            ...
            <relatedInfo type="reuseInformation">
                ...
                <xsl:choose>
                    <xsl:when
                        test="count(relatedidentifiers/relatedidentifier/relatedIdentifer[string-length() > 0]) > 0">
        ...
        <rights>
                <xsl:choose>
                    <xsl:when test="contains(., 'CC')">
                        <xsl:variable name="licensePreFormatted"
                            select="replace(replace(., '_', ' '), 'CCBY', 'CC BY')"/>

                        <xsl:variable name="ccNoNumber_sequence" as="xs:string*">
                            <xsl:analyze-string select="$licensePreFormatted" regex="[^0-9.,]+">
                                <!-- anything but a number or '.'  -->
                                <xsl:matching-substring>
                                    <xsl:if test="string-length(regex-group(0)) > 0">
                                        <xsl:variable name="license"
                                            select="fn:normalize-space(regex-group(0))"/>
                                        <xsl:choose>
                                            <xsl:when test="fn:ends-with(upper-case($license), 'V')">
                                                <xsl:value-of
                                                  select="fn:normalize-space(substring($license, 0, string-length($license) - 0))"
                                                />
                                            </xsl:when>
                                            <xsl:otherwise>
                        ...
                        <xsl:for-each select="$ccNoNumber_sequence">
                            <licence>
                                <xsl:attribute name="type">
                                    <xsl:value-of select="replace(., ' ', '-')"/>
                                </xsl:attribute>
                                <xsl:value-of select="normalize-space($licensePreFormatted)"/>
                            </licence>
                        </xsl:for-each>
                    </xsl:when>
    ...
    <xsl:template match="data" mode="party">
        ...
        <xsl:apply-templates
            select="creators/creator[(string-length(givenname) > 0) or (string-length(familyname) > 0)]"
            mode="party_individual"/>
        <xsl:apply-templates select="creators/creator[(string-length(institution) > 0)]"
            mode="party_institution"/>
        <xsl:apply-templates select="creators/creator[(string-length(organization) > 0)]"
            mode="party_organization"/>

        <xsl:apply-templates
            select="contributors/contributor[(string-length(givenname) > 0) or (string-length(familyname) > 0)]"
            mode="party_individual"/>
        <xsl:apply-templates select="contributors/contributor[(string-length(institution) > 0)]"
            mode="party_institution"/>
        <xsl:apply-templates select="contributors/contributor[(string-length(organization) > 0)]"
            mode="party_organization"/>
                ...
                <xsl:if test="string-length(identifier.orcid) > 0">
                    <identifier type="orcid">
                        <xsl:choose>
                            ...
                            <xsl:otherwise>
                                <xsl:value-of
                                    select="concat('https://orcid.org/', identifier.orcid)"/>
                            </xsl:otherwise>
                        </xsl:choose>
                    </identifier>
                </xsl:if>
                ...
                <xsl:if test="string-length(identifier.scopus) > 0">
                    <identifier type="scopus">
                        <xsl:choose>
                            ...
                            <xsl:otherwise>
                                <xsl:value-of
                                    select="concat('https://www.scopus.com/authid/detail.uri?authorId=', identifier.scopus)"
                                />
                            </xsl:otherwise>
                        </xsl:choose>
                    </identifier>
    ...
    <xsl:template match="data" mode="collection_citation">
        <xsl:if test="count(./*[starts-with(local-name(), 'identifier')][string-length() > 0]) > 0">
            ...
            <citationInfo>
                <citationMetadata>
                    <identifier>
                        ...
                        <xsl:choose>
                            <!-- Use doi only if there is one, otherwise us issn, otherwise use whatever is next -->
                            <xsl:when
                                test="string-length(identifier.doi[string-length() > 0][1]) > 0">
                                <xsl:attribute name="type">
                            ...
                            <xsl:when
                                test="string-length(identifier.issn[string-length() > 0][1]) > 0">
                                <xsl:attribute name="type">
                                    <xsl:value-of select="'issn'"/>
                                </xsl:attribute>
                                <xsl:value-of select="identifier.issn"/>
                            </xsl:when>
                            <xsl:otherwise>
                                <xsl:variable name="identifierFallBackNode" select="./*[starts-with(local-name(), 'identifier')][string-length() > 0][1]" as="node()"/>
                                <xsl:if test="not(empty($identifierFallBackNode))">
                                    <xsl:choose>
                                        <xsl:when test="contains(local-name($identifierFallBackNode), 'other')">
                                            <xsl:choose>
                                                <xsl:when test="contains($identifierFallBackNode, 'http')">
                                                  <xsl:attribute name="type">
                                            ...
                                            <xsl:value-of select="$identifierFallBackNode"/>
                                        </xsl:when>
                                        <xsl:otherwise>
                                            <xsl:attribute name="type">
                                                <xsl:choose>
                                                    <xsl:when test="contains(local-name($identifierFallBackNode), 'identifier.')">
                                                        <xsl:value-of select="substring-after(local-name($identifierFallBackNode), 'identifier.')"/>
                                                    </xsl:when>
                                                    <xsl:otherwise>
                    ...
                    <xsl:variable name="maxOrder" as="xs:integer">
                        <xsl:choose>
                            <xsl:when test="count(creators/creator/order[. castable as xs:integer]/xs:integer(.)) > 0">
                                <xsl:value-of select="fn:max(creators/creator/order[. castable as xs:integer]/xs:integer(.))"/>
                            </xsl:when>
                                <xsl:otherwise>
                                    <xsl:value-of select="xs:integer(count(creators/creator))"/> <!-- hoping this will mean that contributor will be after the creators even if they don't have sequence provided -->
                                </xsl:otherwise>
                        </xsl:choose>
                    </xsl:variable>
                    ...
                    <xsl:for-each select="contributors/contributor">
                        <xsl:if
                            test="(string-length(familyname) > 0) or (string-length(organization) > 0)">
