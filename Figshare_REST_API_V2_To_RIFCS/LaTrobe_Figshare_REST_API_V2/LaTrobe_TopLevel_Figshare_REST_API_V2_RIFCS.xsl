...
<xsl:stylesheet 
    ...
    <xsl:template match="custom_fields[contains(name, 'Contributor nameIdentifier') and contains(value, 'ror')]" mode="collection_custom_handling">
        <xsl:message select="'Handling custom fields where name is [Contributor nameIdentifier] - for La Trobe'"/>
    ...
    <xsl:template match="custom_fields" mode="collection_custom_handling">
        <xsl:message select="'Handling custom fields where name is not [Contributor nameIdentifier] - for La Trobe'"></xsl:message>
    </xsl:template>    
