...
<xsl:stylesheet version="2.0" 
    ...
    <xsl:param name="global_originatingSource" select="'European Centre for Medium-Range Weather Forecasts (ECMWF)'"/> <!-- Only used as originating source if organisation name cannot be determined from Point Of Contact -->
    <xsl:param name="global_group" select="'European Centre for Medium-Range Weather Forecasts (Hosted at National Computational Infrastructure)'"/> 
    ...
    <xsl:template match="*:MD_Metadata" mode="TOP_LEVEL">
        <xsl:message>NCI_ECMWF_toplevel_aggregating</xsl:message>
