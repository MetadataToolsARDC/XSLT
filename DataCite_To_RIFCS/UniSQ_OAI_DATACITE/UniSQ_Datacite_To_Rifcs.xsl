    ...
    <!--xsl:template match="resource" mode="resourceSubType">
        ...
        <xsl:variable name="inferredPrefix" as="xs:string*">
            <xsl:for-each select="tokenize($global_hesanda_identifier_strings, '\|')">
                ...
                <xsl:if
                    test="count($resourceNode/relatedIdentifiers/relatedIdentifier[contains(lower-case(.), $testString)])">
        ...
        <xsl:choose>
            <xsl:when test="boolean(string-length($inferredPrefix[1]))">
