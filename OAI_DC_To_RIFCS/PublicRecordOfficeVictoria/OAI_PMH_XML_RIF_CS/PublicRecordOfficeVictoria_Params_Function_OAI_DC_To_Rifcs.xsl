...
<xsl:stylesheet version="2.0"
    ...
    <xsl:import href="PROV_OAI_DC_To_RIFCS_Function.xsl"/>
    ...
    <xsl:template match="record">
        <xsl:message select="concat('num oai_dc:dc party element: ', count(metadata/oai_dc:dc[contains(lower-case(dc:type), 'activity')]))"/>
        <xsl:apply-templates select="metadata/oai_dc:dc[contains(lower-case(dc:type), 'activity')]" mode="activity"/>
    </xsl:template>
